...
./scripts/svd2groov_multi.py svd/*.svd -o include/stm32/ --verbose --stats
...
- `include/stm32/registers/` - Shared register templates deduplicated across all MCUs
- `include/stm32/<mcu>/` - MCU aggregate headers (one file per MCU)
- `svd/` - STM32 SVD source files
- `scripts/` - Code generation scripts

### Code Generation

Headers are auto-generated by `svd2groov_multi.py` from STM32 SVD files. The script:
1. Parses all 126 SVD files at once
2. Computes register signatures based on field layouts
3. Deduplicates identical register definitions (~13.8x reduction, 5132 unique templates)
4. Groups templates by peripheral type into 119 header files in `registers/`
