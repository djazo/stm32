/* File autogenerated with svd2groov */
#pragma once

#include <groov/groov.hpp>
#include <stm32/common/access.hpp>
#include <stm32/common/bittypes.hpp>

namespace stm32::regs {

// sai1_sai_aclrfr_v1: SAI_ACLRFR
// Used by: SAI1
template <stdx::ct_string name,
          std::uint32_t   baseaddress,
          std::uint32_t   offset>
using sai1_sai_aclrfr_v1_tt =
  groov::reg<name,
             std::uint32_t,
             baseaddress + offset,
             access::rw,
               groov::field<"reserved1", std::uint32_t, 31, 7, access::ro>,
               groov::field<"clfsdet", bool, 6, 6>,
               groov::field<"cafsdet", bool, 5, 5>,
               groov::field<"ccnrdy", bit_ready, 4, 4>,
               groov::field<"reserved0", bool, 3, 3, access::ro>,
               groov::field<"cwckcfg", bool, 2, 2>,
               groov::field<"cmutedet", bool, 1, 1>,
               groov::field<"covrudr", bool, 0, 0>>;

// sai1_sai_acr1_v1: SAI_ACR1
// Used by: SAI1
template <stdx::ct_string name,
          std::uint32_t   baseaddress,
          std::uint32_t   offset>
using sai1_sai_acr1_v1_tt =
  groov::reg<name,
             std::uint32_t,
             baseaddress + offset,
             access::rw,
               groov::field<"reserved3", std::uint8_t, 31, 24, access::ro>,
               groov::field<"mckdiv", std::uint8_t, 23, 20>,
               groov::field<"nodiv", bool, 19, 19>,
               groov::field<"reserved2", bool, 18, 18, access::ro>,
               groov::field<"dmaen", bit_enable, 17, 17>,
               groov::field<"saiaen", bit_enable, 16, 16>,
               groov::field<"reserved1", std::uint8_t, 15, 14, access::ro>,
               groov::field<"outdriv", bool, 13, 13>,
               groov::field<"mono", bool, 12, 12>,
               groov::field<"syncen", std::uint8_t, 11, 10>,
               groov::field<"ckstr", bool, 9, 9>,
               groov::field<"lsbfirst", bit_reset, 8, 8>,
               groov::field<"ds", std::uint8_t, 7, 5>,
               groov::field<"reserved0", bool, 4, 4, access::ro>,
               groov::field<"prtcfg", std::uint8_t, 3, 2>,
               groov::field<"mode", std::uint8_t, 1, 0>>;

// sai1_sai_acr2_v1: SAI_ACR2
// Used by: SAI1
template <stdx::ct_string name,
          std::uint32_t   baseaddress,
          std::uint32_t   offset>
using sai1_sai_acr2_v1_tt =
  groov::reg<name,
             std::uint32_t,
             baseaddress + offset,
             access::rw,
               groov::field<"reserved0", std::uint16_t, 31, 16, access::ro>,
               groov::field<"comp", std::uint8_t, 15, 14>,
               groov::field<"cpl", bool, 13, 13>,
               groov::field<"mutecnt", std::uint8_t, 12, 7>,
               groov::field<"muteval", bool, 6, 6>,
               groov::field<"mute", bool, 5, 5>,
               groov::field<"tris", bool, 4, 4>,
               groov::field<"fflush", bool, 3, 3>,
               groov::field<"fth", std::uint8_t, 2, 0>>;

// sai1_sai_adr_v1: SAI_ADR
// Used by: SAI1
template <stdx::ct_string name,
          std::uint32_t   baseaddress,
          std::uint32_t   offset>
using sai1_sai_adr_v1_tt =
  groov::reg<name,
             std::uint32_t,
             baseaddress + offset,
             access::rw,
               groov::field<"data", std::uint32_t, 31, 0>>;

// sai1_sai_afrcr_v1: SAI_AFRCR
// Used by: SAI1
template <stdx::ct_string name,
          std::uint32_t   baseaddress,
          std::uint32_t   offset>
using sai1_sai_afrcr_v1_tt =
  groov::reg<name,
             std::uint32_t,
             baseaddress + offset,
             access::rw,
               groov::field<"reserved1", std::uint16_t, 31, 19, access::ro>,
               groov::field<"fsoff", bool, 18, 18>,
               groov::field<"fspol", bool, 17, 17>,
               groov::field<"fsdef", bool, 16, 16, access::ro>,
               groov::field<"reserved0", bool, 15, 15, access::ro>,
               groov::field<"fsall", std::uint8_t, 14, 8>,
               groov::field<"frl", std::uint8_t, 7, 0>>;

// sai1_sai_aim_v1: SAI_AIM
// Used by: SAI1
template <stdx::ct_string name,
          std::uint32_t   baseaddress,
          std::uint32_t   offset>
using sai1_sai_aim_v1_tt =
  groov::reg<name,
             std::uint32_t,
             baseaddress + offset,
             access::rw,
               groov::field<"reserved0", std::uint32_t, 31, 7, access::ro>,
               groov::field<"lfsdetie", bit_enable, 6, 6>,
               groov::field<"afsdetie", bit_enable, 5, 5>,
               groov::field<"cnrdyie", bit_ready, 4, 4>,
               groov::field<"freqie", bit_enable, 3, 3>,
               groov::field<"wckcfgie", bit_enable, 2, 2>,
               groov::field<"mutedetie", bit_enable, 1, 1>,
               groov::field<"ovrudrie", bit_enable, 0, 0>>;

// sai1_sai_aslotr_v1: SAI_ASLOTR
// Used by: SAI1
template <stdx::ct_string name,
          std::uint32_t   baseaddress,
          std::uint32_t   offset>
using sai1_sai_aslotr_v1_tt =
  groov::reg<name,
             std::uint32_t,
             baseaddress + offset,
             access::rw,
               groov::field<"sloten", std::uint16_t, 31, 16>,
               groov::field<"reserved1", std::uint8_t, 15, 12, access::ro>,
               groov::field<"nbslot", std::uint8_t, 11, 8>,
               groov::field<"slotsz", std::uint8_t, 7, 6>,
               groov::field<"reserved0", bool, 5, 5, access::ro>,
               groov::field<"fboff", std::uint8_t, 4, 0>>;

// sai1_sai_asr_v1: SAI_ASR
// Used by: SAI1
template <stdx::ct_string name,
          std::uint32_t   baseaddress,
          std::uint32_t   offset>
using sai1_sai_asr_v1_tt =
  groov::reg<name,
             std::uint32_t,
             baseaddress + offset,
             access::ro,
               groov::field<"reserved1", std::uint16_t, 31, 19>,
               groov::field<"flth", std::uint8_t, 18, 16>,
               groov::field<"reserved0", std::uint16_t, 15, 7>,
               groov::field<"lfsdet", bool, 6, 6>,
               groov::field<"afsdet", bool, 5, 5>,
               groov::field<"cnrdy", bit_ready, 4, 4>,
               groov::field<"freq", bool, 3, 3>,
               groov::field<"wckcfg", bool, 2, 2>,
               groov::field<"mutedet", bool, 1, 1>,
               groov::field<"ovrudr", bool, 0, 0>>;

// sai1_sai_bclrfr_v1: SAI_BCLRFR
// Used by: SAI1
template <stdx::ct_string name,
          std::uint32_t   baseaddress,
          std::uint32_t   offset>
using sai1_sai_bclrfr_v1_tt =
  groov::reg<name,
             std::uint32_t,
             baseaddress + offset,
             access::rw,
               groov::field<"reserved1", std::uint32_t, 31, 7, access::ro>,
               groov::field<"clfsdet", bool, 6, 6>,
               groov::field<"cafsdet", bool, 5, 5>,
               groov::field<"ccnrdy", bit_ready, 4, 4>,
               groov::field<"reserved0", bool, 3, 3, access::ro>,
               groov::field<"cwckcfg", bool, 2, 2>,
               groov::field<"cmutedet", bool, 1, 1>,
               groov::field<"covrudr", bool, 0, 0>>;

// sai1_sai_bcr1_v1: SAI_BCR1
// Used by: SAI1
template <stdx::ct_string name,
          std::uint32_t   baseaddress,
          std::uint32_t   offset>
using sai1_sai_bcr1_v1_tt =
  groov::reg<name,
             std::uint32_t,
             baseaddress + offset,
             access::rw,
               groov::field<"reserved3", std::uint8_t, 31, 24, access::ro>,
               groov::field<"mckdiv", std::uint8_t, 23, 20>,
               groov::field<"nodiv", bool, 19, 19>,
               groov::field<"reserved2", bool, 18, 18, access::ro>,
               groov::field<"dmaen", bit_enable, 17, 17>,
               groov::field<"saiben", bit_enable, 16, 16>,
               groov::field<"reserved1", std::uint8_t, 15, 14, access::ro>,
               groov::field<"outdriv", bool, 13, 13>,
               groov::field<"mono", bool, 12, 12>,
               groov::field<"syncen", std::uint8_t, 11, 10>,
               groov::field<"ckstr", bool, 9, 9>,
               groov::field<"lsbfirst", bit_reset, 8, 8>,
               groov::field<"ds", std::uint8_t, 7, 5>,
               groov::field<"reserved0", bool, 4, 4, access::ro>,
               groov::field<"prtcfg", std::uint8_t, 3, 2>,
               groov::field<"mode", std::uint8_t, 1, 0>>;

// sai1_sai_bcr2_v1: SAI_BCR2
// Used by: SAI1
template <stdx::ct_string name,
          std::uint32_t   baseaddress,
          std::uint32_t   offset>
using sai1_sai_bcr2_v1_tt =
  groov::reg<name,
             std::uint32_t,
             baseaddress + offset,
             access::rw,
               groov::field<"reserved0", std::uint16_t, 31, 16, access::ro>,
               groov::field<"comp", std::uint8_t, 15, 14>,
               groov::field<"cpl", bool, 13, 13>,
               groov::field<"mutecnt", std::uint8_t, 12, 7>,
               groov::field<"muteval", bool, 6, 6>,
               groov::field<"mute", bool, 5, 5>,
               groov::field<"tris", bool, 4, 4>,
               groov::field<"fflush", bool, 3, 3>,
               groov::field<"fth", std::uint8_t, 2, 0>>;

// sai1_sai_bdr_v1: SAI_BDR
// Used by: SAI1
template <stdx::ct_string name,
          std::uint32_t   baseaddress,
          std::uint32_t   offset>
using sai1_sai_bdr_v1_tt =
  groov::reg<name,
             std::uint32_t,
             baseaddress + offset,
             access::rw,
               groov::field<"data", std::uint32_t, 31, 0>>;

// sai1_sai_bfrcr_v1: SAI_BFRCR
// Used by: SAI1
template <stdx::ct_string name,
          std::uint32_t   baseaddress,
          std::uint32_t   offset>
using sai1_sai_bfrcr_v1_tt =
  groov::reg<name,
             std::uint32_t,
             baseaddress + offset,
             access::rw,
               groov::field<"reserved1", std::uint16_t, 31, 19, access::ro>,
               groov::field<"fsoff", bool, 18, 18>,
               groov::field<"fspol", bool, 17, 17>,
               groov::field<"fsdef", bool, 16, 16, access::ro>,
               groov::field<"reserved0", bool, 15, 15, access::ro>,
               groov::field<"fsall", std::uint8_t, 14, 8>,
               groov::field<"frl", std::uint8_t, 7, 0>>;

// sai1_sai_bim_v1: SAI_BIM
// Used by: SAI1
template <stdx::ct_string name,
          std::uint32_t   baseaddress,
          std::uint32_t   offset>
using sai1_sai_bim_v1_tt =
  groov::reg<name,
             std::uint32_t,
             baseaddress + offset,
             access::rw,
               groov::field<"reserved0", std::uint32_t, 31, 7, access::ro>,
               groov::field<"lfsdetie", bit_enable, 6, 6>,
               groov::field<"afsdetie", bit_enable, 5, 5>,
               groov::field<"cnrdyie", bit_ready, 4, 4>,
               groov::field<"freqie", bit_enable, 3, 3>,
               groov::field<"wckcfgie", bit_enable, 2, 2>,
               groov::field<"mutedetie", bit_enable, 1, 1>,
               groov::field<"ovrudrie", bit_enable, 0, 0>>;

// sai1_sai_bslotr_v1: SAI_BSLOTR
// Used by: SAI1
template <stdx::ct_string name,
          std::uint32_t   baseaddress,
          std::uint32_t   offset>
using sai1_sai_bslotr_v1_tt =
  groov::reg<name,
             std::uint32_t,
             baseaddress + offset,
             access::rw,
               groov::field<"sloten", std::uint16_t, 31, 16>,
               groov::field<"reserved1", std::uint8_t, 15, 12, access::ro>,
               groov::field<"nbslot", std::uint8_t, 11, 8>,
               groov::field<"slotsz", std::uint8_t, 7, 6>,
               groov::field<"reserved0", bool, 5, 5, access::ro>,
               groov::field<"fboff", std::uint8_t, 4, 0>>;

// sai1_sai_bsr_v1: SAI_BSR
// Used by: SAI1
template <stdx::ct_string name,
          std::uint32_t   baseaddress,
          std::uint32_t   offset>
using sai1_sai_bsr_v1_tt =
  groov::reg<name,
             std::uint32_t,
             baseaddress + offset,
             access::ro,
               groov::field<"reserved1", std::uint16_t, 31, 19>,
               groov::field<"flth", std::uint8_t, 18, 16>,
               groov::field<"reserved0", std::uint16_t, 15, 7>,
               groov::field<"lfsdet", bool, 6, 6>,
               groov::field<"afsdet", bool, 5, 5>,
               groov::field<"cnrdy", bit_ready, 4, 4>,
               groov::field<"freq", bool, 3, 3>,
               groov::field<"wckcfg", bool, 2, 2>,
               groov::field<"mutedet", bool, 1, 1>,
               groov::field<"ovrudr", bool, 0, 0>>;

} // namespace stm32::regs
