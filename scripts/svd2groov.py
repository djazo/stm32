...
def generate_field_line(
    ...
    return f'               groov::field<"{f.name}", {cpp_type}, {f.msb}, {f.lsb}{access_str}>{comma}'
...
def generate_aggregate_entry(peripheral: Peripheral) -> str:
    ...
    for reg in peripheral.registers:
        ...
        reg_names.append((template_name, reg_upper, reg.offset))
    ...
    for i, (template_name, reg_upper, offset) in enumerate(reg_names):
        if i == len(reg_names) - 1:
            # Last entry - close both the template and the group
            lines.append(f'                 {template_name}<"{reg_upper}", baseaddress, {format_offset(offset)}>>;')
        else:
            lines.append(f'                 {template_name}<"{reg_upper}", baseaddress, {format_offset(offset)}>,')
